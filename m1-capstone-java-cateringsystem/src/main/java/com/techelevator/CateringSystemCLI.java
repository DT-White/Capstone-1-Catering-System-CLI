...
public class CateringSystemCLI {
    ...
    public void run() {
        ...
        while (true) {
			...
			*/
            menu.showMainMenu();
            int userSelection = cateringSystem.checkForValidMenuSelection(menu.readUserSelection("Please enter selection by number: "));
            if (userSelection == 0) {
                ...
                while (true) {
                    ...
                    userSelection = cateringSystem.checkForValidMenuSelection(menu.readUserSelection("Please enter selection by number: "));
                    if (userSelection == 0) {
                        menu.showCaseMessage("Invalid entry.");
                    } else if (userSelection == 1) {
                        menu.showCaseMessage(bank.addMoney(cateringSystem.checkForValidMoneyEntry()));
                    } else if (userSelection == 2) {
                        menu.showInventory(inventory, cateringSystem);
                        menu.showCaseMessage(menu.tryUserSelectedAddToCart(cateringSystem));
                    } else if (userSelection == 3) {
                        cateringSystem.completeTransaction(inventoryFileReader, logFileWriter);
                        break;
                    }
                }
            } else if (userSelection == 3) {
